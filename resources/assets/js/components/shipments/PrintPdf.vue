<template>
<div id="home" class="container mt-5">
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card card-body">
                <div class="form-group">
                    <v-layout row>
                        <v-flex xs12 sm5 offset-sm1>
                            <v-text-field v-model="form.start_date" :type="'date'" color="blue darken-2" label="Start Date" required></v-text-field>
                        </v-flex>
                        <v-flex xs12 sm5 offset-sm1>
                            <v-text-field v-model="form.end_date" :type="'date'" color="blue darken-2" label="End Date" required></v-text-field>
                        </v-flex>
                    </v-layout>
                </div>
                <v-btn raised color="primary" @click="getScheduled" :disabled="loading" :loading="loading">Download PDF</v-btn>
            </div>
        </div>
    </div>
</div>
</template>

<script>
import jsPDF from 'jsPDF'
import VueBarcode from "vue-barcode";

export default {
    name: 'print',
    components: {
        barcode: VueBarcode,
    },
    data() {
        return {
            name: '',
            Scheduled: [],
            form: {},
            errors: [],
            loading: false,
        }
    },
    methods: {
        getScheduled() {
            this.loading = true
            axios.post('getScheduled', this.$data.form)
                .then((response) => {
                    this.loading = false
                    this.Scheduled = response.data
                    this.download()
                })
                .catch((error) => {
                    this.loading = false
                    this.errors = error.response.data.errors
                })
        },
        download() {
            let pdfName = 'SpeedBall';
            var doc = new jsPDF();
            // doc.text(this.name, 10, 10);
            var doc = new jsPDF();
            var imgData = 'data:image/png;base64,'

            for (let i = 0; i < this.Scheduled.length; i++) {
                doc.addImage(imgData, 5, 2, 20, 20);
                doc.setFontSize(12)
                const element = this.Scheduled[i];
                doc.setFont("times");
                doc.setFontType("normal");
                // doc.text(20, 120, 'Back to left');

                doc.rect(0, 20, 80, 45);
                doc.rect(110, 20, 100, 45);
                doc.rect(0, 75, 70, 40);
                doc.rect(75, 75, 70, 40);
                doc.rect(150, 75, 70, 40);

                doc.line(0, 133, 1000, 133);
                doc.line(0, 143, 1000, 143);

                doc.setLineWidth(0.1);
                doc.line(70, 133, 70, 143); // vertical line
                doc.line(160, 133, 160, 143); // vertical line
                // doc.setLineWidth(0.5);
                // doc.line(70, 25, 70, 60);

                // doc.setLineWidth(0.5);
                // doc.line(5, 25, 60, 25);
                doc.text(5, 30, 'SpeedBall Courier Services.');
                doc.text(5, 40, 'Location: ' + element['sender_city']);
                doc.text(5, 50, 'Phone: ' + element['sender_phone']);
                doc.text(5, 60, 'Email: info@speedballcourier.com');
                // doc.setLineWidth(0.5);
                // doc.line(5, 65, 60, 65);

                // doc.text(105, 80, 'This is centred text.', null, null, 'center');
                doc.text(115, 30, 'Booking Date: ' + element['booking_date'], null, null, 'left');
                doc.text(115, 40, 'From: ' + element['sender_address'], null, null, 'left');
                doc.text(115, 50, 'To: ' + element['client_address'], null, null, 'left');
                // doc.text(200, 110, 'And some more', null, null, 'right');
                doc.text(5, 80, 'Sender Details :');
                doc.text(5, 90, 'Name: SpeedBall Courier Services');
                doc.text(5, 100, 'Phone: ' + element['sender_phone']);
                doc.text(5, 110, 'Email: info@speedballcourier.com');

                doc.text(80, 80, 'Client Details :', null, null, 'left');
                doc.text(80, 90, 'Name: ' + element['client_name'], null, null, 'left');
                doc.text(80, 100, 'Phone: ' + element['client_phone'], null, null, 'left');
                doc.text(80, 110, 'Email: ' + element['client_email'], null, null, 'left');

                doc.text(160, 80, 'Shipment Details :', null, null, 'left');
                doc.text(160, 90, 'WayBill No: ' + element['bar_code'], null, null, 'left');
                doc.text(160, 100, 'Derivery Date:: ' + element['derivery_date'], null, null, 'left');
                // doc.text(105, 80, 'Client Details :', null, null, 'center');

                doc.text(5, 130, 'Product Description');
                // doc.text(85, 130, 'Weight', null, null, 'center');
                doc.text(90, 130, 'Price', null, null, 'left');
                doc.text(170, 130, 'Quantity', null, null, 'left');
                // doc.text(180, 130, 'Total', null, null, 'right');

                // doc.text(200, 80, 'Shipment Details :', null, null, 'right');
                // doc.text(140, 140, element['cod_amount'], null, null, 'right');
                // doc.text(180, 140, element['amount_ordered'], null, null, 'right');

                doc.text(5, 140, '' + element['bar_code']);
                // doc.text(85, 140, element['weight'], null, null, 'center');
                doc.text(90, 140, '' + element['cod_amount'], null, null, 'left');
                doc.text(170, 140, '' + element['amount_ordered'], null, null, 'left');

                // for (let j = 0; j < element.products.length; j++) {
                //     const products = element.products[j];
                //     doc.text(5, 140, products['product_name']);
                //     doc.text(85, 140, products['weight'], null, null, 'center');
                //     doc.text(140, 140, products['price'], null, null, 'left');
                //     doc.text(180, 140, products['quantity'], null, null, 'left');
                //     // doc.text(180, 140, products['quantity'], null, null, 'left');
                // }
                doc.text(5, 150, 'Special Instructions: ' + element['speciial_instruction']);

                doc.text(5, 160, 'NOTE: ');
                doc.text(5, 170, 'Clients are requested to pay through M-PESA TILL NUMBER - 877838 ');
                doc.text(5, 180, '(If asked to pay cash please cal 0728 492 446 or 0799 869 844) ');

                doc.text(5, 200, 'Authorizer Signature____________________________');
                doc.text(200, 200, 'Customer Signature____________________________', null, null, 'right');
                doc.addPage()
            }
            doc.save(pdfName + '.pdf');
        }
    },
    mounted() {},
}
</script>
